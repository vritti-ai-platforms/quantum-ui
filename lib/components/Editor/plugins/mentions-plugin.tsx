...
import { Command, CommandGroup, CommandItem, CommandList } from '../editor-ui/command';
...
export function MentionsPlugin(): JSX.Element | null {
  const { onMentionSearch } = useEditorConfig();
  const [editor] = useLexicalComposerContext();

  const [queryString, setQueryString] = useState<string | null>(null);

  const results = useMentionLookupService(queryString, onMentionSearch);
  ...
  const options = useMemo(
    () =>
      results
        .map((result) => new MentionTypeaheadOption(result.name, <CircleUserRoundIcon className="size-4" />))
